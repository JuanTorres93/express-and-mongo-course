...
const createSendToken = (user, statusCode, req, res) => {
  ...
  const cookieOptions = {
    ...
    //secure: true, // only send on encrypted connection (https)
...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  let token;
  // Authorize user using JWT token in the header
  if (
  ...
  // Authorize user using JWT token in the cookie
...
exports.isLoggedIn = async (req, res, next) => {
  // token will always be in the cookie, the header
  // is only for API
  if (req.cookies.jwt) {
    try {
      ...
      // Pass the user to the pug template
      res.locals.user = currentUser;
    } catch (error) {
      return next();
    }
